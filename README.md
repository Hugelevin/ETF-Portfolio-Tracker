...
Dashboard quotes come only from the canonical 1W refresh/cache. Opening another chart range or Portfolio Insights never changes that quote or its daily baseline. ETF daily change uses Yahoo's previous-session metadata, not the last candle of an arbitrary chart. Current price/time use Yahoo's timestamped regular-market quote. This release invalidates older market-cache identities so incorrect daily baselines cannot survive an upgrade; portfolios and settings remain intact. Independent devices still need the same orders and a successful refresh to agree; offline or older cached prices can differ.

Historical ranges request 5-minute points for ETF 1D/1W, hourly points for ETF 1M, and daily points for ETF 3M/1Y/MAX. If Yahoo silently coarsens MAX to weekly/monthly candles, the tracker requests one year of daily observations instead. For ETFs with missing daily history, it tries hourly data for the requested symbol and retains the last observation per day. MAX therefore means maximum usable history from these requests, not guaranteed full lifetime history. Risk Statistics displays actual coverage dates. UMMEPSA uses daily NAV points at every range because no intraday NAV exists.

**ANAU market-data mapping:** The displayed ANAU holding and its Milan order identity remain unchanged. At the user's request, all quotes, Today values, sparklines and chart ranges use Yahoo `ANAV.DE` instead of the incomplete `ANAU-ETFP.MI` feed. [Deutsche Börse identifies ANAV as ISIN IE000QDFFK00, trading in EUR](https://live.deutsche-boerse.com/en/etf/bnp-paribas-easy-ii-nasdaq-100-ucits-etf-usd-acc), the same fund/share class as ANAU. Xetra prices and daily changes can differ from Milan/Moneybase. The mapping is internal, with no extra disclaimer in the interface; it requires the exact verified ISIN, currency, type and original listing. Old ANAU quote/history caches are invalidated together, not orders. If Yahoo's 1D window is empty before opening, the chart uses only the latest available session from its 5D feed, without changing sources. Redeploy the Worker with `pnpm worker:deploy` to allow `ANAV.DE` before publishing this frontend; custom `ALLOWED_SYMBOLS` lists must include it too.

The default **Price** chart shows a weighted average purchase-price baseline when it falls inside the visible market range; a distant cost basis never flattens short-term price movement. Its tooltip shows only date and price. The optional **Holding Value** view compares historical holding value with invested cost, excludes broker fees and starts at the first purchase, so it never plots a misleading zero before shares were owned. Its tooltip also shows holding value and change. Charts use a padded data range rather than forcing the Y-axis to zero, so normal market movement remains readable.

Each phone holding card includes a 7-day price sparkline. Holding details and the purchase form behave like in-app pages: the browser Back gesture closes them before leaving the dashboard. Orders use compact touch-friendly cards on phones and a table on larger screens.
...
Invested capital is shares multiplied by purchase price; broker fees remain separate. Market return is current value minus that capital. Net return additionally subtracts broker fees. Annualised return uses dated purchase cash flows (XIRR). When current quotes have different dates (for example, daily fund NAV versus intraday ETFs), it uses actual historical prices on the latest day shared by every EUR holding, with a compact “As of” date. Orders after that date are excluded from both capital flows and share quantities. It does not backdate current quotes, carry missing prices forward, or change current dashboard values. Without a shared valuation or valid cash flows it remains unavailable.
