...
for (const instrument of portfolio.instruments) {
  const base = proxy
    ? `${proxy.replace(/\/$/, "")}/yahoo/chart?symbol=${encodeURIComponent(instrument.yahooSymbol)}&range=5d&interval=${instrument.assetType === "FUND" ? "1d" : "5m"}`
    : `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(instrument.yahooSymbol)}?range=5d&interval=${instrument.assetType === "FUND" ? "1d" : "5m"}&events=history`;
  try {
    ...
    const venueOk = instrument.assetType === "FUND" ||
      (instrument.exchange === "Xetra" && /xetra/i.test(venue)) ||
      (instrument.exchange === "Milan" && /(milan|italy)/i.test(venue));
    const typeOk = instrument.assetType === "FUND" ? /fund/i.test(String(meta?.instrumentType)) : meta?.instrumentType === "ETF";
