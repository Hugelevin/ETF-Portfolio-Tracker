...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <main id="main">
      ...
      {!positions.length ? <section className="empty-state"><div className="empty-icon"><ChartNoAxesCombined /></div><p className="eyebrow">Get Started</p><h2>Build Your Portfolio</h2><p>Add an order or import your portfolio JSON file to begin tracking your investments.</p><div><button className="button primary" onClick={openPurchase}><Plus /> Add First Order</button><button className="button secondary" onClick={() => { persist(SAMPLE_PORTFOLIO); setNotice("Public VanEck sample loaded."); }}>Load Public Sample</button></div></section> : <HoldingsTable positions={positions} loading={loading} errors={errors} sparklineHistory={(instrumentId) => chartRecords[cacheKey(instrumentId, "1W")]?.history ?? chartRecords[cacheKey(instrumentId, "1M")]?.history ?? []} onSelect={(position) => openDetail(position.instrument.id)} />}
      {positions.length > 0 && <PortfolioInsights positions={positions} baseCurrency={portfolio.baseCurrency} loading={loading.size > 0} getRecord={(instrumentId, range) => chartRecords[cacheKey(instrumentId, range)] ?? null} onRange={(range) => void refreshAll(range, false)} />}
    </main>
