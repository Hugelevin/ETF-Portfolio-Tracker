...
import { formatDate, formatDateTime, formatMoney, formatNumber, formatPercentInBrackets } from "../format";
    ...
    <section ref={keyboard.dialogRef} onKeyDown={keyboard.onKeyDown} className="modal detail-modal" role="dialog" aria-modal="true" aria-labelledby="detail-title">
      ...
      <div className="detail-body">
        {instrument.assetType === "FUND" ? <div className="quote-strip">
          ...
          <div><p>Performance</p><dl className="period-performance" aria-live="polite"><div><dt>1W</dt><dd className={weekly ? (weekly.value < 0 ? "negative-text" : "positive-text") : undefined}>{weekly ? formatPercentInBrackets(weekly.percentage) : unavailablePerformance}</dd></div><div><dt>1M</dt><dd className={monthly ? (monthly.value < 0 ? "negative-text" : "positive-text") : undefined}>{monthly ? formatPercentInBrackets(monthly.percentage) : unavailablePerformance}</dd></div></dl></div>
