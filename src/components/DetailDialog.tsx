...
export function DetailDialog({ position, record, loading, error, onClose, onRange, onLotSave, onLotDelete, onManualPrice }: Props) {
  ...
  return <div className="modal-backdrop detail-backdrop" role="presentation"><section ref={keyboard.dialogRef} onKeyDown={keyboard.onKeyDown} className="modal detail-modal" role="dialog" aria-modal="true" aria-labelledby="detail-title"><header className="detail-header"><div className="detail-identity"><span className="ticker-mark large">{instrument.ticker.slice(0, 4)}</span><div><p className="eyebrow">{instrument.assetType === "FUND" ? "Accumulating money-market fund" : "Accumulating UCITS ETF"}</p><h2 id="detail-title">{instrument.ticker} · {instrument.name}</h2><p>{instrument.isin} · {instrument.exchange} · {instrument.currency}</p></div></div><button className="icon-button" aria-label="Close details" onClick={onClose}><X /></button></header>
    <div className="detail-body">
      ...
      {instrument.assetType === "FUND" && <p className="fund-note">This fund publishes a daily NAV. Intraday data does not exist; 1D and 1W controls use the latest available daily NAV points.</p>}
      ...
      <section className="manual-panel"><div><h3>Manual {instrument.assetType === "FUND" ? "NAV" : "price"}</h3><p>Use only when provider data is unavailable. It is visibly labelled and never presented as live.</p></div><form onSubmit={setManual}><label>Price / NAV<input name="manualPrice" type="number" min="0.000001" step="any" required /></label><label>As-of date<input name="manualDate" type="date" required /></label><button className="button secondary" type="submit">Save manual value</button></form></section>
    </div>
