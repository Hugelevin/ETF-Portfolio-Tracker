...
export function HoldingSparkline({ history, ticker }: Props) {
  ...
  const area = `${line} L${(width - inset).toFixed(1)},${(height - inset).toFixed(1)} L${inset},${(height - inset).toFixed(1)} Z`;
  const rising = values.at(-1)! >= values[0]!;
  const colour = rising ? "#19725c" : "#b34a4a";

  return <span className="holding-sparkline">
    <span>7D</span>
    <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`${ticker} 7-day price trend, ${rising ? "up" : "down"}`} preserveAspectRatio="none">
