    ...
    <div className="table-shell">
      <table>
        ...
        <tbody>{positions.map((position) => <tr key={position.instrument.id}>
          ...
          <td data-label="Price / NAV"><strong>{position.quote ? formatMoney(position.quote.price, position.instrument.currency) : position.instrument.assetType === "FUND" && position.instrument.annualYieldPercentage !== undefined ? `${formatNumber(position.instrument.annualYieldPercentage)}% APY` : "Unavailable"}</strong>{position.dailyChange !== null && <small className={`today-change ${direction(position.dailyChange)}`}>Today {position.dailyChange >= 0 ? "▲" : "▼"} {formatMoney(position.dailyChange, position.instrument.currency)} · {formatPercent(position.dailyChangePercentage)}</small>}{position.costBasisWarning ? <span className="status stale">Review Cost Basis</span> : position.quote ? <><StatusBadge quote={position.quote} loading={loading.has(position.instrument.id)} error={errors[position.instrument.id]} /><small className="timestamp">As at {formatDateTime(position.quote.asOf)}</small>{position.instrument.assetType === "FUND" && position.instrument.annualYieldPercentage !== undefined && <small className="apy-context">Current APY {formatNumber(position.instrument.annualYieldPercentage)}%</small>}</> : position.instrument.assetType === "FUND" && position.instrument.annualYieldPercentage !== undefined ? <span className="status manual">APY Estimate</span> : <StatusBadge quote={position.quote} loading={loading.has(position.instrument.id)} error={errors[position.instrument.id]} />}{position.costBasisWarning && <small className="cost-basis-warning">Re-import the corrected portfolio JSON.</small>}</td>
