...
export function HoldingsTable({ positions, loading, errors, sparklineHistory, onSelect }: Props) {
    ...
    {compact
      ...
      : <div className="table-shell holdings-desktop"><table>
        ...
        <tbody>{positions.map((position) => <tr key={position.instrument.id}>
          ...
          <td><div className="row-actions"><button className="icon-button" aria-label={`Open ${position.instrument.ticker} details`} onClick={() => onSelect(position)}><ChevronRight /></button></div></td>
        </tr>)}</tbody>
      </table></div>}
  </section>;
}
