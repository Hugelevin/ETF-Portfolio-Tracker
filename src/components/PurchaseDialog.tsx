...
export function PurchaseDialog({ onClose, onSave }: { onClose: () => void; onSave: (instrument: Instrument, lot: PurchaseLot) => void }) {
  ...
  const keyboard = useDialogKeyboard(onClose, "[aria-label='Close order form']");
  ...
  function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!instrument || !Number.isFinite(shares) || shares <= 0 || !Number.isFinite(pricePerShare) || pricePerShare <= 0 || !Number.isFinite(fees) || fees < 0 || !/^\d{4}-\d{2}-\d{2}$/.test(purchaseDate)) {
      setError(!instrument
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section ref={keyboard.dialogRef} onKeyDown={keyboard.onKeyDown} className="modal purchase-modal" role="dialog" aria-modal="true" aria-labelledby="purchase-title">
      <header><div><p className="eyebrow">New Order</p><h2 id="purchase-title">Add Order</h2></div><button className="icon-button modal-close" aria-label="Close order form" onClick={onClose}><X /></button></header>
      <form onSubmit={submit}>
        {!selectedInstrument ? <div className="instrument-picker">
          <fieldset className="instrument-options"><legend>Select Instrument</legend>{VERIFIED_INSTRUMENTS.map((item) => <label className="instrument-option" key={item.id}><input type="radio" name="instrument" value={item.id} checked={false} onChange={() => { setInstrumentId(item.id); setError(""); }} /><InstrumentLogo instrument={item} /><span><strong>{item.ticker} <em>{item.assetType === "FUND" ? "Fund" : "ETF"}</em></strong><small>{item.name}</small><small>{item.isin} · {item.exchange} · {item.currency}</small></span></label>)}</fieldset>
          ...
          <button ref={changeButtonRef} type="button" className="button secondary compact-button" onClick={() => { setInstrumentId(""); setError(""); }}>Change</button>
        </div>}
        ...
        {selectedInstrument && <footer><button type="button" className="button secondary" onClick={onClose}>Cancel</button><button type="submit" className="button primary">Add Order</button></footer>}
      </form>
    </section>
  </div>;
}
