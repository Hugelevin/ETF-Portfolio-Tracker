...
export function SummaryCards({ summary, positions, updatedAt }: { summary: PortfolioSummary; positions: PositionMetrics[]; updatedAt: string | null }) {
  ...
  return <section className={`summary${summary.dailyChange !== null && summary.dailyChange < 0 ? " loss" : ""}`} aria-labelledby="summary-title">
    ...
    <div className="summary-grid">
      ...
      <article className={`metric-card ${tone(summary.dailyChange)}`}><div className="metric-label"><Gauge aria-hidden="true" /><p>Today</p></div><strong>{formatMoney(summary.dailyChange)}</strong><small>{summary.dailyChangePositions ? `${formatPercent(summary.dailyChangePercentage)}${summary.dailyChangePositions < summary.pricedPositions ? " · Partial" : ""}` : "Unavailable"}</small></article>
    </div>
