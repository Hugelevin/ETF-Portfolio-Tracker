...
export function SummaryCards({ summary, positions }: { summary: PortfolioSummary; positions: PositionMetrics[] }) {
  ...
  return <section className="summary" aria-labelledby="summary-title">
    <div className="summary-heading">
      ...
      <span className={`coverage ${incomplete ? "warning" : ""}`}>{summary.pricedPositions} of {summary.baseCurrencyPositions} EUR positions priced{nonEur.length ? ` · ${nonEur.length} non-EUR separate` : ""}</span>
    </div>
    <div className="summary-grid">
      <article className="metric-card"><span className="metric-icon"><Landmark aria-hidden="true" /></span><p>Total invested</p><strong>{formatMoney(summary.totalInvested)}</strong><small>Cost basis, including broker fees</small></article>
      ...
      <article className={`metric-card ${tone(summary.profitLoss)}`}><span className="metric-icon">{summary.profitLoss >= 0 ? <ArrowUpRight aria-hidden="true" /> : <ArrowDownRight aria-hidden="true" />}</span><p>Unrealised return</p><strong>{summary.pricedPositions ? formatMoney(summary.profitLoss) : "Unavailable"}</strong><small>{summary.pricedPositions ? formatPercent(summary.profitLossPercentage) : "No priced positions"}</small></article>
      <article className={`metric-card ${tone(summary.dailyChange)}`}><span className="metric-icon"><Gauge aria-hidden="true" /></span><p>Daily change</p><strong>{formatMoney(summary.dailyChange)}</strong><small>{summary.dailyChangePositions ? `Against previous close · ${summary.dailyChangePositions} of ${summary.pricedPositions} priced` : "Previous close unavailable"}</small></article>
    </div>
