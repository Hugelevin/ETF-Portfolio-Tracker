...
export function SummaryCards({ summary, positions }: { summary: PortfolioSummary; positions: PositionMetrics[] }) {
  ...
  return <section className="summary" aria-labelledby="summary-title">
    ...
    <div className="summary-grid">
      ...
      <article className={`metric-card ${tone(summary.marketReturn)}`}><span className="metric-icon">{summary.marketReturn >= 0 ? <ArrowUpRight aria-hidden="true" /> : <ArrowDownRight aria-hidden="true" />}</span><p>Market Return</p><strong>{summary.pricedPositions ? formatMoney(summary.marketReturn) : "Unavailable"}</strong><small>{summary.pricedPositions ? `${formatPercent(summary.marketReturnPercentage)} · Net After Fees ${formatMoney(summary.profitLoss)}` : "No Valued Positions"}</small></article>
